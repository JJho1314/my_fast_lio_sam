...
float min_range;
float max_range;
std::string globalSurfMap_dirctory;
std::string globalSurfMap_pcd;
int area_size;
int margin;
...
void dynamic_load_map(const float pose[])
{
    if (margin >= 0)
    {
        ...
        *globalMap = *laserCloudSurfFromMapDS;

        ikdtree.reconstruct(laserCloudSurfFromMapDS->points);
