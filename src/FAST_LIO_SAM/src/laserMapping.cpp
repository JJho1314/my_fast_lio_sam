...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pubMemoryCost = nh.advertise<std_msgs::UInt32>("/fastlio_memory", 1000);
    //------------------------------------------------------------------------------------------------------
    ...
    while (status)
    {
        ...
        while (getline(file, line))
        {
            ...
            {
                // cout << "VmRSS: " << line.substr(line.find(" ")) << endl;
                int memory_cost = stoi(line.substr(line.find(" ")));
                memory_cost /= 1024; // Mb
                std_msgs::UInt32 uint32_msg;
                uint32_msg.data = memory_cost;
                pubMemoryCost.publish(uint32_msg);
                break;
            }
        }
