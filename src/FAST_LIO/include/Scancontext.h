...
using InvKeyTree = KDTreeVectorOfVectorsAdaptor< KeyMat, float >; // 自定义的用于vector的KD-tree
...
public:
    // hyper parameters ()
    const double LIDAR_HEIGHT = 2.0; //雷达的安装高度 lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord) / if you use robot-coord-transformed lidar scans, just set this as 0.

    const int    PC_NUM_RING = 20; // 径向分成20份 in the original paper (IROS 18)
    const int    PC_NUM_SECTOR = 60; // 环向分成60份 60 in the original paper (IROS 18)
    const double PC_MAX_RADIUS = 80.0;//径向最大距离半径R=80m // 80 meter max in the original paper (IROS 18)
    const double PC_UNIT_SECTORANGLE = 360.0 / double(PC_NUM_SECTOR);
    const double PC_UNIT_RINGGAP = PC_MAX_RADIUS / double(PC_NUM_RING);

    // tree
    const int    NUM_EXCLUDE_RECENT = 50; //简单的把最近的50个关键帧不参与回环检测 simply just keyframe gap, but node position distance-based exclusion is ok. 
    const int    NUM_CANDIDATES_FROM_TREE = 10; //候选关键帧数量 10 is enough. (refer the IROS 18 paper)
    ...
    // config 
    const int    TREE_MAKING_PERIOD_ = 10; //闭环检测的搜索树重构间隔 i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost / in the LeGO-LOAM integration, it is synchronized with the loop detection callback (which is 1Hz) so it means the tree is updated evrey 10 sec. But you can use the smaller value because it is enough fast ~ 5-50ms wrt N.
    int          tree_making_period_conter = 0;//上面参数的计数

    // data 
    std::vector<double> polarcontexts_timestamp_; // optional.
    std::vector<Eigen::MatrixXd> polarcontexts_;   // SC
    std::vector<Eigen::MatrixXd> polarcontext_invkeys_;// ringkey
    std::vector<Eigen::MatrixXd> polarcontext_vkeys_;//sectorKey

    KeyMat polarcontext_invkeys_mat_;  //ringkey ->vector
    KeyMat polarcontext_invkeys_to_search_; // 除掉最近的NUM_EXCLUDE_RECENT个关键帧后的vector
