...
public:
    ...
    std::queue<sensor_msgs::PointCloud2ConstPtr> keyframeQueue;
    ...
    {
        Eigen::Quaternionf qwl(_laserOdometry->pose.pose.orientation.w, _laserOdometry->pose.pose.orientation.x, _laserOdometry->pose.pose.orientation.y, _laserOdometry->pose.pose.orientation.z);
        Eigen::Vector3d eulerAngle = qwl.cast<double>().matrix().eulerAngles(2, 1, 0);   //yaw pitch roll 单位：弧度

        transformTobeMapped[0] = eulerAngle(2);   // roll
        transformTobeMapped[1] = eulerAngle(1);   // pitch
        transformTobeMapped[2] = eulerAngle(0);   // yaw
        transformTobeMapped[3] = _laserOdometry->pose.pose.position.x;   // x
        transformTobeMapped[4] = _laserOdometry->pose.pose.position.y;   // y
        transformTobeMapped[5] = _laserOdometry->pose.pose.position.z;   // z                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           
    ...
    {                               
        mBuf.lock();
        keyframeQueue.push(_laserCloudFullRes);
        mBuf.unlock();
