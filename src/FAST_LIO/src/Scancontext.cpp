...
//计算_sc1当前帧，_sc2对齐后的闭环候选帧的距离=1-相似度
double SCManager::distDirectSC ( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    for ( int col_idx = 0; col_idx < _sc1.cols(); col_idx++ )
    {
        ...
        double sector_similarity = col_sc1.dot(col_sc2) / (col_sc1.norm() * col_sc2.norm());// 两个向量的cos<a,b>作为相似度

        sum_sector_similarity = sum_sector_similarity + sector_similarity;
        num_eff_cols = num_eff_cols + 1;
    }
    
    double sc_sim = sum_sector_similarity / num_eff_cols; //平均相似度
    return 1.0 - sc_sim;

} // distDirectSC

// 遍历shift值，使得_vkey1和_vkey2对齐
int SCManager::fastAlignUsingVkey( MatrixXd & _vkey1, MatrixXd & _vkey2)
{
    ...
    {
        MatrixXd vkey2_shifted = circshift(_vkey2, shift_idx); //平移

        MatrixXd vkey_diff = _vkey1 - vkey2_shifted;//平移后的插值
...
/**
 * @details 计算sc1和sc2的距离和对齐所需的shfit
 * @param _sc1 当前帧
 * @param _sc2 候选帧
 */
std::pair<double, int> SCManager::distanceBtnScanContext( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    MatrixXd vkey_sc1 = makeSectorkeyFromScancontext( _sc1 ); //计算Sectorkey
    MatrixXd vkey_sc2 = makeSectorkeyFromScancontext( _sc2 );
    int argmin_vkey_shift = fastAlignUsingVkey( vkey_sc1, vkey_sc2 );// 计算对齐所需的shfit值

    const int SEARCH_RADIUS = round( 0.5 * SEARCH_RATIO * _sc1.cols() ); // 搜索半径 a half of search range 
    //以shift为中心，SEARCH_RADIUS为半径设置搜索空间
    ...
    {
        MatrixXd sc2_shifted = circshift(_sc2, num_shift);//对齐后的sc2
...
/**
 * @details 将点云转换成Scancontext，并保存ringkey和sectorkey
 * @param _scan_down:关键帧
 */
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down )
{
    Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1 
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc );
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc );//没有用到，后面又重新算了一遍
...
/**
 * @details 检测闭环关键帧ID
 * @return std::pair<int, float>,int:闭环关键帧ID， float：
 */ 
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    // knn search
    std::vector<size_t> candidate_indexes( NUM_CANDIDATES_FROM_TREE ); //候选关键帧索引 
    std::vector<float> out_dists_sqr( NUM_CANDIDATES_FROM_TREE ); //平方距离
     ...
     */
    if( min_dist < SC_DIST_THRES )//检测是否满足阈值
    {
        loop_id = nn_idx; 
